...
def load_controller():
    path = Path(__file__).resolve().parents[1] / "controller" / "vpnhub-controller.py"
    spec = importlib.util.spec_from_file_location("vpnhub_controller", path)
...
def base_state():
    return {
        ...
        "peers": [{
            ...
            "public_key": key(1),
            "preshared_key": key(2),
            "allowed_ips": ["10.250.1.10/32", "192.168.10.0/24"],
        }],
        "sites": [{
            "id": 1,
            "name": "Site A",
            "ranges": ["10.250.1.0/24", "192.168.10.0/24"],
        }],
...
def test_validate_state_accepts_valid_wireguard_keys():
    controller = load_controller()
    state = controller.validate_state(base_state())
    assert state["listen_port"] == 51820
    assert state["peers"][0]["allowed_ips"][1] == "192.168.10.0/24"
...
def test_nftables_has_admin_and_site_isolation_rules():
    controller = load_controller()
    state = controller.validate_state(base_state())
    rules = controller.render_nftables(state, table_exists=False)
